...
module lcd_integrado (
    ...
    output wire LCD_BLON,           // backlight
	 ...
	 // Saídas para controle do processador
    output reg [1:0] PROG_SEL,
    output reg PROG_LOAD_EN,
    output reg PREEMPTION_MODE,   // Novo: indica modo preemptivo
	 output reg program_selected
);
...
reg preemption_selected = 1'b0;   // Flag: modo selecionado 
    ...
    if (enable_pulse) begin
        case (state)
            ...
            21: begin
                ...
                lcd_data_reg <= 8'h00;
					 if (SW[0] == 1 && KEY[0]==1) begin
							PREEMPTION_MODE <= 1'b0;  // Modo não-preemptivo
							state <= 22; // troca de tela
					 end
					 if (SW[1] == 1 && KEY[0]==1) begin
							PREEMPTION_MODE <= 1'b1;  // Modo preemptivo (Round-Robin)
            ...
            26: begin
                ...
                lcd_rs_reg <= 0;
                lcd_data_reg <= 8'h00;
					 if (SW[1] == 0 && SW[0] == 1 && KEY[0]==1) begin
							LED_PROG <= 4'b0001;
							PROG_SEL <= 2'b01;       // Inicia com programa 1
							program_selected <= 1'b1;  // NOVO
					 end
					 if (SW[1] == 1 && SW[0] == 0 && KEY[0]==1) begin
							LED_PROG <= 4'b0010;
							PROG_SEL <= 2'b10;       // Inicia com programa 2
							program_selected <= 1'b1;  // NOVO
					 end
					 if (SW[1] == 1 && SW[0] == 1 && KEY[0]==1) begin
							LED_PROG <= 4'b0011;
							PROG_SEL <= 2'b11;       // Inicia com programa 3
							program_selected <= 1'b1;  // NOVO
					 end
					 if (SW[3] == 1 && SW[2] == 0 && SW[1] == 0 && SW[0] == 0 && KEY[0]==1) begin
							LED_PROG <= 4'b0000;
							PROG_SEL <= 2'b00;
							program_selected <= 1'b0;  // NOVO
            ...
            27: begin
					lcd_data_reg <= 8'h01;
					lcd_rs_reg <= 0; lcd_en_reg <= 1;
					PROG_LOAD_EN <= 1'b1;    // Habilita carregamento de programas
					program_selected <= 1'b1;
					PROG_SEL <= 2'b01;       // Inicia com programa 0
					state <= 28;
				end
            ...
            31: begin
                ...
                lcd_data_reg <= 8'h00;
					 if (SW[3] == 1 && SW[2] == 0 && SW[1] == 0 && SW[0] == 0 &&KEY[0]==1) begin
					 		PROG_LOAD_EN <= 1'b0;    // Desabilita carregamento de programas
							program_selected <= 1'b0;
							PROG_SEL <= 2'b00;       // Inicia com programa 0
							state <= 13; // troca para tela 2 (selecao de tipo de execucao)
					 end
            end
